...
anywhere else in the string is the index that you return. This approach works, but it's not optimal. Are there any data structures that you can use to improve the time complexity of this approach?

Hint 3
Hash tables are very commonly used to keep track of frequencies. Build a hash table, where every key is a character in the string and every value is the corresponding character's frequency in the input string. You can traverse the entire string once to fill the hash table, and then with a second traversal
through the string (not a nested traversal), you can use the hash table's constant-time lookups to find the first character with a frequency of 1 .
...
function solution1(string) {
    ...
    for (let i in string) {
        if (charObj[string[i]]) {
            charObj[string[i]].count += 1;
            charObj[string[i]].index = Number(i);
        } else {
            charObj[string[i]] = {
                count: 1,
                index: Number(i)
    ...
    for (let i in charObj) {
        if (charObj[i].count === 1) return charObj[i].index
    }
    return -1
}
...
function solution2(string) {
    ...
    for (let i in string) {
        if (charObj[string[i]] === 1) return charObj[string[i]]
    }
    return -1
}
