...
function solution1(array) {
    ...
    for(let i in array){
        if(Number(i)===0) product=1
        leftProducts.push(product)
        product*=array[i]
    }
    for (let i = array.length-1; i >=0; i--) {
        if(Number(i)===array.length-1) product=1
        rightProducts.push(product)
        product*=array[i]
    }
    leftProducts.forEach((num,i) => {
        products.push(leftProducts[i]*rightProducts[rightProducts.length-1-Number(i)])
    });
